...
Usage:
  PYTHONPATH=.. python tools/test_mavftp_hardware.py /dev/ttyACM0
...
def run(device: str, baud: int, component: int) -> None:  # pylint: disable=too-many-branches,too-many-locals,too-many-statements
    ...
    try:
        ...
        if result.error_code != FtpError.Success:
            raise RuntimeError(f"status failed: {result.error_code.name}")
        result = ftp.cmd_set(["debug", "0"])
        print(f"set debug: {result.error_code.name}", flush=True)
        if result.error_code != FtpError.Success:
            raise RuntimeError(f"set debug failed: {result.error_code.name}")
        result = ftp.cmd_cancel()
        print(f"cancel: {result.error_code.name}", flush=True)
        if result.error_code != FtpError.Success:
            raise RuntimeError(f"cancel failed: {result.error_code.name}")
        ...
        with tempfile.NamedTemporaryFile(prefix="mavftp_crc_", suffix=".bin") as local_file:
            ...
            independent_crc = zlib.crc32(payload, 0xFFFFFFFF) ^ 0xFFFFFFFF
            if local_crc != independent_crc:
                raise RuntimeError(
                    f"local CRC algorithm mismatch: "
                    f"0x{local_crc:08x} != 0x{independent_crc:08x}"
                )
            ...
            result = ftp.cmd_crclocal([local_file.name])
            if result.error_code != FtpError.Success:
                raise RuntimeError(
                    f"direct local CRC failed: {result.error_code.name}"
                )
            result = ftp.cmd_crclocal([local_file.name + ".missing"])
            print(f"missing local CRC: {result.error_code.name}", flush=True)
            if result.error_code != FtpError.FailToOpenLocalFile:
                raise RuntimeError(
                    f"missing local CRC was not rejected: {result.error_code.name}"
                )
        ...
        with tempfile.TemporaryDirectory(prefix="mavftp_crccmp_") as temp_dir:
            ...
            different_file = f"{temp_dir}/{remote.rsplit('/', 1)[-1]}"
            with open(different_file, "wb") as compare_file:
                compare_file.write(payload + b"different")
            result = ftp.cmd_crccmp([f"{temp_dir}/*.bin", "/APM"])
            print(
                f"crccmp different: {result.error_code.name} "
                f"{ftp.crccmp_results}",
                flush=True,
            )
            if result.error_code != FtpError.Success or ftp.crccmp_results != ["DIFFER"]:
                raise RuntimeError(
                    f"CRC difference was not detected: {result.error_code.name} "
                    f"{ftp.crccmp_results}"
                )

            missing_file = f"{temp_dir}/mavftp_hwtest_missing.bin"
            with open(missing_file, "wb") as compare_file:
                compare_file.write(payload)
            result = ftp.cmd_crccmp([missing_file, "/APM"])
            print(
                f"crccmp missing: {result.error_code.name} "
                f"{ftp.crccmp_results}",
                flush=True,
            )
            if result.error_code != FtpError.Success or ftp.crccmp_results != ["MISSING"]:
                raise RuntimeError(
                    f"CRC missing file was not reported: {result.error_code.name} "
                    f"{ftp.crccmp_results}"
                )

        result = ftp.cmd_get([remote, "-"])
        if result.error_code != FtpError.Success:
            raise RuntimeError(f"get setup failed: {result.error_code.name}")
        ...
        if result.error_code != FtpError.Success:
            raise RuntimeError(
                f"callback-failure upload setup failed: {result.error_code.name}"
            )
        ...
        for cycle in range(3):
            ...
            if result.error_code != FtpError.Success:
                raise RuntimeError(
                    f"chained upload setup failed: {result.error_code.name}"
                )
            result = ftp.process_ftp_reply("Put", timeout=60)
            ...
            if result.error_code != FtpError.Success:
                raise RuntimeError(
                    f"chained get setup failed: {result.error_code.name}"
                )
            result = ftp.process_ftp_reply("Get", timeout=60)
            ...
            if result.error_code != FtpError.Success:
                raise RuntimeError(
                    f"chained renamed get setup failed: {result.error_code.name}"
                )
            result = ftp.process_ftp_reply("Get", timeout=60)
        ...
        with tempfile.TemporaryDirectory(prefix="mavftp_params_") as temp_dir:
            ...
            if result.error_code != FtpError.Success:
                raise RuntimeError(
                    f"getparams setup failed: {result.error_code.name}"
                )
